...
public interface ShippingRuleJpaRepository
        extends
            JpaRepository<ShippingRuleEntity, String>,
            JpaSpecificationExecutor<ShippingRuleEntity> {
    ...
    @Query(value = "SELECT r.*, c.name AS carrier_name, c.code AS carrier_code, c.logo_url, c.active AS carrier_active, "
            ...
            + "WHERE r.active = true AND c.active = true " + "AND r.zone = :zone "
    ...
    List<ShippingRuleEntity> findApplicableRules(@Param("zone") String zone, @Param("weight") BigDecimal weight,
