...
PaginationDtoOut<Invoice> result =invoiceUseCase.findByUserId(userId,page,size,sortBy,ascending);
return ResponseEntity.ok(PaginationMapper.map(result,invoiceApiMapper::toDto));
    }
...
if (orderId != null)filters.put("orderId",orderId);
PaginationDtoOut<Invoice> result =invoiceUseCase.findAll(filters,page,size,sortBy,ascending);
return ResponseEntity.ok(PaginationMapper.map(result,invoiceApiMapper::toDto));
    }
