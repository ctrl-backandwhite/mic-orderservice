...
public class TrackingController {
        ...
        return cjOrderRepository.findByOrderId(orderId).map(cjOrder -> {
            ...
            try {
                ...
                return ResponseEntity.<Map<String, Object>>ok(Map.of("trackingNumber", info.getTrackingNumber(),
                        "logisticName", nullSafe(info.getLogisticName()), "trackingFrom",
                        nullSafe(info.getTrackingFrom()), "trackingTo", nullSafe(info.getTrackingTo()),
                        "trackingStatus", nullSafe(info.getTrackingStatus()), "deliveryDay",
                        nullSafe(info.getDeliveryDay()), "deliveryTime", nullSafe(info.getDeliveryTime()),
                        "lastMileCarrier", nullSafe(info.getLastMileCarrier()), "lastTrackNumber",
    ...
    public ResponseEntity<TrackingEventDtoOut> addEvent(@RequestHeader(AppConstants.HEADER_NX036_AUTH) String nxAuth,
