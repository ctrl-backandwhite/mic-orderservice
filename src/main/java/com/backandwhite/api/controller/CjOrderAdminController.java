...
public class CjOrderAdminController {
    ...
    public ResponseEntity<CjOrderDtoOut> getByOrderId(
    ...
    public ResponseEntity<CjOrderDtoOut> syncStatus(
            @RequestHeader(AppConstants.HEADER_NX036_AUTH) String nxAuth,
            @PathVariable String orderId) {
    ...
    public ResponseEntity<CjOrderDtoOut> submitToCj(
            @RequestHeader(AppConstants.HEADER_NX036_AUTH) String nxAuth,
            @PathVariable String orderId) {
    ...
    private CjOrderDtoOut toDto(CjOrder cjOrder) {
        return CjOrderDtoOut.builder()
                .id(cjOrder.getId())
                .orderId(cjOrder.getOrderId())
                .cjOrderId(cjOrder.getCjOrderId())
                .shipmentOrderId(cjOrder.getShipmentOrderId())
                .cjOrderStatus(cjOrder.getCjOrderStatus())
                .trackNumber(cjOrder.getTrackNumber())
                .logisticName(cjOrder.getLogisticName())
                .productInfoList(cjOrder.getProductInfoList())
                .lastSyncedAt(cjOrder.getLastSyncedAt())
                .errorCount(cjOrder.getErrorCount())
                .lastError(cjOrder.getLastError())
                .createdAt(cjOrder.getCreatedAt())
                .updatedAt(cjOrder.getUpdatedAt())
                .build();
    }
}
