...
PaginationDtoOut<ShippingCarrier> result =shippingTaxUseCase.findAllCarriers(page,size,sortBy,ascending);
return ResponseEntity.ok(PaginationMapper.map(result,shippingTaxApiMapper::toCarrierDto));
    }
...
public ResponseEntity<PaginationDtoOut<ShippingRuleDtoOut>> findAllRules(
            ...
            @Parameter(description = "Campodeorden") @RequestParam(defaultValue = "createdAt") String sortBy,
            @Parameter(description = "Ascendente") @RequestParam(defaultValue = "true") boolean ascending) {
PaginationDtoOut<ShippingRule> result =shippingTaxUseCase.findAllRules(page,size,sortBy,ascending);
return ResponseEntity.ok(PaginationMapper.map(result,shippingTaxApiMapper::toRuleDto));
    }
